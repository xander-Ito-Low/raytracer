...
{
    maxa = Vertex(-1000,-1000,-1000);
    mina = Vertex(1000,1000,1000);
    cout<<mina.x<<" "<<mina.y<<" "<<mina.z;
    cout<<"\n";
  this->do_construct(file, transform);
}
...
void PolyMesh::do_construct(char *file, Transform *transform)
{
  ...
  for (i = 0; i < triangle_count; i += 1)
  {
    ...
    //only uncomment if using smaller teapot
    triangle[i][0] -= 1;
    triangle[i][1] -= 1;
    triangle[i][2] -= 1;
...
float PolyMesh::findMax(float a,float b,float c){
    float max;
    if(b>a && b>c){
        max = b;
    }
...
float PolyMesh::findMin(float a,float b,float c){
    float min;
    if(b<a && b<c){
        min = b;
    }
...
float PolyMesh::findMax(float a,float b){
    float max;
    if(b>a){
        max = b;
    }
...
float PolyMesh::findMin(float a,float b){
    float min;
    if(b<a){
        min = b;
    }
...
//Find the maximum and minimum vertecies of the polymesh
void PolyMesh::findMinMax(int which_triangle){
    Vertex v1 = vertex[triangle[which_triangle][0]];
    Vertex v2 = vertex[triangle[which_triangle][1]];
    Vertex v3 = vertex[triangle[which_triangle][2]];
    Vertex TriMin;
    Vertex TriMax;
    
    TriMin.x = findMin(v1.x,v2.x,v3.x);
    TriMin.y = findMin(v1.y,v2.y,v3.y);
    TriMin.z = findMin(v1.z,v2.z,v3.z);
    
    TriMax.x = findMax(v1.x,v2.x,v3.x);
    TriMax.y = findMax(v1.y,v2.y,v3.y);
    TriMax.z = findMax(v1.z,v2.z,v3.z);
    
    mina.x = findMin(mina.x,TriMin.x);
    mina.y = findMin(mina.y,TriMin.y);
    mina.z = findMin(mina.z,TriMin.z);
    
    maxa.x = findMax(maxa.x,TriMax.x);
    maxa.y = findMax(maxa.y,TriMax.y);
    maxa.z = findMax(maxa.z,TriMax.z);
...
void PolyMesh::compute_face_normal(int which_triangle, Vector &normal)
{
  ...
  //Add a part here to compute the minimum and maximum corners for construction of bounding box
  findMinMax(which_triangle);
...
void PolyMesh::intersection(Ray ray, Hit &hit)
{
  ...
  hit.flag = false;
//Bounding box will accelerate the ray tracing.
BoundingBox *bounding = new BoundingBox(mina, maxa);

  // Check each triangle, find closest if any intersecion
    
    if(bounding->intersection(ray)){
        for(i = 0; i < triangle_count; i += 1)
        {
